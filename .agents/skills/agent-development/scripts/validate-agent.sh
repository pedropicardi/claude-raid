...
# Check description field — handles multiline YAML (>, |, and inline)
# YAML block scalars (> and |) continue until a non-blank line with less indentation
DESCRIPTION=$(echo "$FRONTMATTER" | awk '
  ...
  END { if (collecting) print result }
')
...
if [ -z "$DESCRIPTION" ]; then
  ...
  # Check for triggering pattern ("Use this/as the agent when..." or team-dispatched patterns)
  if ! echo "$DESCRIPTION" | grep -qiE 'use (this agent|as the main agent)|raid teammate'; then
    echo "⚠️  description should include triggering context (e.g. 'Use this agent when...' or team role)"
    ((warning_count++))
  fi
fi
...
if [ -z "$MODEL" ]; then
  ...
  case "$MODEL" in
    inherit|sonnet|opus|haiku|claude-opus-4-6|claude-sonnet-4-6|claude-haiku-4-5-20251001|claude-sonnet-4-5-20250514)
      # Valid model (aliases and explicit model IDs)
      ;;
    *)
...
if [ -z "$COLOR" ]; then
  ...
  case "$COLOR" in
    blue|cyan|green|yellow|magenta|red|purple|orange)
      # Valid color
      ;;
    *)
